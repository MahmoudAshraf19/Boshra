...
class PrayerSchedulerService {
  ...
  Future<void> scheduleTestNotification(String langCode) async {
    ...
    final settings = await PrayerSettings.load();
    await _schedulePrayer(999999, 'Test / اختبار', now, settings, langCode);
  }
  ...
  Future<void> _scheduleDayPrayers(DateTime date, PrayerTimesModel model, PrayerSettings settings) async {
    ...
    if (settings.enabledFajr && fajrTime != null) {
      await _schedulePrayer(dayBaseId + 1, getPrayerName('Fajr'), fajrTime, settings, langCode);
    }
    if (settings.enabledDhuhr && dhuhrTime != null) {
      await _schedulePrayer(dayBaseId + 2, getPrayerName('Dhuhr'), dhuhrTime, settings, langCode);
    }
    if (settings.enabledAsr && asrTime != null) {
      await _schedulePrayer(dayBaseId + 3, getPrayerName('Asr'), asrTime, settings, langCode);
    }
    if (settings.enabledMaghrib && maghribTime != null) {
      await _schedulePrayer(dayBaseId + 4, getPrayerName('Maghrib'), maghribTime, settings, langCode);
    }
    if (settings.enabledIsha && ishaTime != null) {
      await _schedulePrayer(dayBaseId + 5, getPrayerName('Isha'), ishaTime, settings, langCode);
    }
  }

  Future<void> _schedulePrayer(int baseId, String prayerName, DateTime time, PrayerSettings settings, String langCode) async {
